...
int setup_uring(uring_ctx_t *ctx) {
  ...
  p.sq_thread_idle = UINT_MAX / 1000; // keep polling thread alive

  /* allocate separate SVM regions for the rings and SQEs */
  ...
  /* separate buffers for the ring metadata and for the SQE array */
  ...
  fprintf(stderr, "setup: sq_ring_ptr=%p sqe_ptr=%p\n", ctx->sq_ring_ptr,
          sqe_mem);
  fprintf(stderr, "setup: sring_head=%p tail=%p mask=%p array=%p\n",
          ctx->sring_head, ctx->sring_tail, ctx->sring_mask, ctx->sring_array);
  ...
  fprintf(stderr, "setup: ring_fd=%d head=%u tail=%u\n", ctx->ring_fd,
          *ctx->sring_head, *ctx->sring_tail);
...
void uring_process_completions(uring_ctx_t *ctx) {
  ...
  while (head != tail) {
    ...
    fprintf(stderr, "cqe idx=%u res=%d\n", idx, cqe->res);
    head++;
  }
...
void uring_flush(uring_ctx_t *ctx) {
  /* Notify polling thread of new submissions */
