...
int setup_uring(uring_ctx_t *ctx) {
  ...
  memset(&p, 0, sizeof(p));
  /* operate io_uring without a polling thread */
  ...
  /* No polling thread: nothing to wake here */
...
void uring_flush(uring_ctx_t *ctx) {
  /* Submit queued SQEs and wait for at least one completion */
  int ret = io_uring_enter(ctx->ring_fd, QUEUE_DEPTH, 1, IORING_ENTER_GETEVENTS);
  if (ret < 0)
